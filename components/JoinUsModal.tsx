import React from 'react';
import type { Language } from '../types';
import { locales } from '../locales';
import { UsersIcon } from './icons';

interface JoinUsModalProps {
  isOpen: boolean;
  onClose: () => void;
  language: Language;
}

const JoinUsModal: React.FC<JoinUsModalProps> = ({ isOpen, onClose, language }) => {
  const t = locales[language];

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-gray-800 border border-gray-700 rounded-xl shadow-2xl w-full max-w-2xl flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-600">
          <h2 className="font-title text-2xl text-cyan-400 flex items-center gap-2">
            <UsersIcon className="h-6 w-6"/>
            {t.joinUsModalTitle}
          </h2>
          <button onClick={onClose} className="text-gray-400 hover:text-white text-3xl leading-none">&times;</button>
        </div>
        <div className="p-6 text-gray-300 space-y-6 leading-relaxed text-center">
            <p>{t.joinUsModalContent}</p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
                 <a 
                    href="https://www.tangibledata.xyz" 
                    target="_blank" 
                    rel="noopener noreferrer" 
                    className="w-full sm:w-auto bg-cyan-600 hover:bg-cyan-700 text-white font-bold py-2 px-4 rounded-lg transition-colors"
                >
                    {t.joinUsModalWebsiteButton}
                 </a>
                 <p className="flex items-center gap-2">
                    {t.joinUsModalEmailLink}
                    <a href="mailto:Hello@tangibledata.xyz" className="text-cyan-400 hover:underline">
                        Hello@tangibledata.xyz
                    </a>
                 </p>
            </div>
        </div>
      </div>
    </div>
  );
};

export default JoinUsModal;
